...
using CrmWebResourcesUpdater.Services.Extensions;

namespace CrmWebResourcesUpdater.Services
{
    /// <summary>
    ...
    public class PublishService : IDisposable
    {
        ...
        private async Task<bool> UpdateWebResourceByFileAsync(Entity webResource, string filePath)
        {
            var settings = await SettingsService.Instance.GetSettingsAsync();
            var connections = settings.CrmConnections;
        ...
        private async System.Threading.Tasks.Task UpdateWebResourceByContentAsync(Entity webResource, string content)
        {
            ...
            var orgService = selectedConnection.GetCrmServiceClient();// await CrmConnectionHelper.GetOrganizationServiceProxyAsync(selectedConnection);
        ...
        private async System.Threading.Tasks.Task<List<Entity>> RetrieveWebResourcesAsync(List<string> webResourceNames = null)
        {
            ...
            var fetchQuery = String.Format(FetchWebResourcesQueryTemplate, selectedConnection.SelectedSolution.SolutionId, filter);
        ...
        private async void OpenCreateWebResourceFormAsync()
        {
            ...
            dialog.OnCreate = async (Entity webResource) =>
            {
                ...
                var service = await connectionDetail.GetCrmServiceClientAsync();
                ...
                try
                {
                    ...
                    this.CreateWebResource(service, webResource, settings.SelectedConnection.SelectedSolution.UniqueName);
        ...
        private async Task<Entity> RetrieveSolutionAsync()
        {
            ...
            query.Criteria.AddCondition("solutionid", ConditionOperator.Equal, selectedConnection.SelectedSolution.SolutionId);
